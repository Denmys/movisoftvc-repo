<!DOCTYPE html>
<html lang="es_ES">

<head>

    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta id="headerContent_keywordsTag" name="keywords" content="Posicionamiento, Web, conocemos, posicionar, resultados, técnicas, primeras, necesario, avanzadas, herramientas, dominio, autoridad, metodología, detalles, máximo, nivel, servicios informáticos, empresa de servicios informáticos, servicios informaticos, informática, servicio integral, servicios consultoría informática, Movisoft, Villa Clara, Cuba, MiPyme, emprendedores, soporte técnico, redes, montaje de redes, SEO, posicionamiento, desarrollo, fabricar, comercializar componentes, diseño gráfico, sistemas operativos. Español">
    </meta>
    <meta id="headerContent_descriptionTag" name="description" content="Ubicamos tu página en los primeros resultados, gracias a técnicas probadas en nosotros mismos, protegemos tu web de penalizaciones en los buscadores.">
    </meta>
    <meta id="headerContent_metaTitleTag" name="Title" content="Posicionamiento - Web - Movisoft">
    </meta>
    <meta id="headerContent_OG_SiteName" property="og:site_name" content="Movisoft">
    </meta>
    <meta id="OG-Image" property="og:image" content="img/movisoft.png">
    </meta>
    <meta property="og:type" content="article" />
    <link rel="icon" type="image/png" sizes="16x16" href="./assets/images/fav-icon/favicon-16x16.png">
    <title>Posicionamiento - Web - Movisoft</title>

    <link rel="stylesheet" href="assets/librerias/bootstrap/css/bootstrap.min.css">
    <link href="https://cdn.jsdelivr.net/npm/remixicon@2.5.0/fonts/remixicon.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@fancyapps/ui@4.0/dist/fancybox.css">
    <link rel="stylesheet" href="https://unpkg.com/aos@next/dist/aos.css" />
    <link rel="stylesheet" href="./assets/css/style.css">

</head>

<body>
    <!--IMPORTANDO ELEMENTO COMUN MENU-->
    <?php require_once "comun/menu.php" ?>

    <?php require_once "modulos/modPosicionamiento.php" ?>

    <!--BOTÓN FLOTANTE-->
    <?php require_once "comun/btnflot.php"?>


    <!--IMPORTANDO ELEMENTO COMÚN FOOTER-->
    <?php require_once "comun/footer.php" ?>

    <script src="assets/librerias/bootstrap/js/bootstrap.bundle.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@fancyapps/ui@4.0/dist/fancybox.umd.js"></script>
    <script src="https://unpkg.com/aos@next/dist/aos.js"></script>
    <script src="./assets/js/main.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/js/all.min.js"></script>
</body>
<?php require_once "comun/modal-os.php"?>
</html>